import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Terms of Service - Legal Terms & Conditions",
  description: "Read RepliFast's terms of service and conditions for using our AI-powered Google review management platform. Legal terms, user responsibilities, and service policies.",
  keywords: [
    "terms of service",
    "legal terms",
    "terms and conditions", 
    "service agreement",
    "user agreement",
    "RepliFast terms",
    "review automation legal"
  ],
  openGraph: {
    title: "Terms of Service - Legal Terms & Conditions | RepliFast",
    description: "Read our terms of service for using RepliFast's AI review management platform. Complete legal terms and user responsibilities.",
    url: "https://replifast.com/terms"
  },
  twitter: {
    title: "Terms of Service - Legal Terms & Conditions | RepliFast", 
    description: "Read our terms of service for using RepliFast's AI review management platform."
  },
  alternates: {
    canonical: "https://replifast.com/terms"
  }
};